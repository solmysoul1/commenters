<template>
    <div class="content custom-bg">
        ...
        <div class="movies-container">
            ...
            <div v-for="(movie, index) in dislike_list" :key="movie.id" class="movie">
...
const dislike_list = ref([])
const showDetails = ref(Array(dislike_list.value.length).fill(false));

    
axios({
    method: 'get',
    url: `${API_URL}/accounts/profile/${userId}/dislikelist/`,
    headers: {
        'Authorization': `Token ${store.token}`
    }
})
.then((response) => {
    dislike_list.value = response.data
    console.log('싫어요한 영화 받아오기 성공!')
})
.catch((error) => {
    console.log(error)
})
...
const moviesJustifyContent = computed(() => {
  return dislike_list.value.length <= 4 ? 'flex-start' : 'center'; // 영화 개수에 따라 중앙 정렬이나 왼쪽 정렬 결정
});
</script>
