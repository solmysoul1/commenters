<template>
    <div class="content custom-bg">
        ...
        <div class="movies-container">
            ...
            <div v-for="(movie, index) in watchlist" :key="movie.id" class="movie">
...
const watchlist = ref([])
const showDetails = ref(Array(watchlist.value.length).fill(false));

    
axios({
    method: 'get',
    url: `${API_URL}/accounts/profile/${userId}/watchlist/`,
    headers: {
...
const moviesJustifyContent = computed(() => {
  return watchlist.value.length <= 4 ? 'flex-start' : 'center'; // 영화 개수에 따라 중앙 정렬이나 왼쪽 정렬 결정
});
</script>
